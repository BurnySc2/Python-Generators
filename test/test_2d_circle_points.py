...
from generators_2d.generators import generate_2d_circle_points
...
def test_circle_points(point_radius, circle_radius):
    if point_radius * 2 > circle_radius:
        assert list(generate_2d_circle_points(point_radius, circle_radius)) == []
        return
    ...
    for point_index, (amount, current_angle, x, y) in enumerate(generate_2d_circle_points(point_radius, circle_radius)):
